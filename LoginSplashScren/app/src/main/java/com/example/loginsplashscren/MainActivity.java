...
public class MainActivity extends AppCompatActivity {
    ...
    private boolean isValidEmail(String correo) {
        return correo.length() > 0 && Patterns.EMAIL_ADDRESS.matcher(correo).matches();
    }
    ...
    private void saveOnPreferences(String email, String password) {
        if (recordar.isChecked()) {
            //to write on sharedpreferences we need new object .edit()
            SharedPreferences.Editor editor = prefs.edit();
            editor.putString("email", email);
            editor.putString("password", password);
            // method not async editor.commit();
            //async method
            editor.apply();
        }
    }
    ...
    private String getUserPassPrefs() {
        //second value is the return if first value does not exist
